...
Definition instantiateOutputPort (pd : PortDeclaration) (outputNetNumbers : list Signal)
                                 : state CavaState unit :=
  cs <- get ;;
  match pd with
  | mkPort name Bit => _ <- outputBit name (hd Vcc outputNetNumbers) ;; ret tt
  | mkPort name (BitVec [n]) => _ <- outputVectorTo0 [n] (firstn n outputNetNumbers) name ;; ret tt
  | _ => ret tt
  end.

Fixpoint instantiateOutputPorts (ports : list PortDeclaration) (outputNetNumbers : list Signal)
                                : state CavaState unit :=
  match ports with
  | [] => ret tt
...
Definition wireUpCircuit (intf : CircuitInterface)
                         `{Flatten (signalTy Signal (mapShape snd (circuitOutputs intf)))}
  ...
  let numberedInputs : signalTy Signal typeShape := numberPort countInputsFrom typeShape in
  instantiateInputPorts (shapeToPortDeclaration inputPort) ;;
  o <- circuit numberedInputs ;;
  let outputNetNumbers : list Signal := flatten o in
